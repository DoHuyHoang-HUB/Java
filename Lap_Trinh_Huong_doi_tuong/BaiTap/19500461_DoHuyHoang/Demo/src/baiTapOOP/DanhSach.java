...
public class DanhSach {
	...
	public DanhSach timKiem(String tuaDe) {
		DanhSach kq = new DanhSach();
		for (NhaXuatBan nhaXuatBan : listNhaXuatBan) {
			if(nhaXuatBan.getTuaDe().equalsIgnoreCase(tuaDe))
				kq.them(nhaXuatBan);
		}
		return kq;
	}
	// Lay so trang lon nhat cua sach
	public int laySoTrangLonNhat() {
		int soTrangMax = 0;
		for (NhaXuatBan nhaXuatBan : listNhaXuatBan) {
			if(nhaXuatBan instanceof Sach)
				if(((Sach) nhaXuatBan).getSoTrang() > soTrangMax)
					soTrangMax = ((Sach) nhaXuatBan).getSoTrang();
		}
		return soTrangMax;
	}
}
